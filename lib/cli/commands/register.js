...
const { handleSimpleRegister } = require('./register-simple');

/**
 * Handle register command - redirect to simplified version
 */
async function handleRegister(projectPath, options) {
  console.error('Debug - handleRegister called with options:', JSON.stringify(options));
  console.error('Debug - options.yes:', options.yes);
  // Use simplified register for better UX
  return handleSimpleRegister(projectPath, options);
}

/**
 * Original complex register (deprecated - kept for reference)
 */
async function handleRegisterComplex(projectPath, options) {
  try {
...
async function handleUnregister(projectId, options) {
  try {
    ...
    if (!options.force) {
      ...
      console.log(chalk.white(`   Name: ${project.config?.name || project.name || 'Unnamed'}`));
...
async function handleList(options) {
  try {
    ...
    if (options.enabledOnly) {
      filteredProjects = projectList.filter(p => p.enabled);
    ...
    for (const project of filteredProjects) {
      const status = project.enabled ? chalk.green('✓ Enabled') : chalk.red('✗ Disabled');
      const priority = `Priority: ${project.config.priority}`;
      const weight = `Weight: ${project.config.weight}`;
      
      console.log(chalk.white(`${chalk.bold(project.config?.name || project.name || 'Unnamed')} (${project.id})`));
      console.log(chalk.gray(`  Path: ${project.path}`));
      console.log(chalk.gray(`  Status: ${status}  ${priority}  ${weight}`));
      console.log(chalk.gray(`  Polling: ${project.config.pollingInterval ? project.config.pollingInterval / 1000 + 's' : 'N/A'}`));
      console.log();
    }
    
    // Show summary
    const enabled = filteredProjects.filter(p => p.enabled).length;
