...
async function handleSimpleRegister(projectPath, options) {
  try {
    ...
    const alreadyRegistered = existingProjects.find(p => p.path === targetPath);
    
    if (alreadyRegistered) {
      console.log(chalk.yellow('⚠️  このプロジェクトは既に登録されています。'));
      console.log(chalk.gray(`ID: ${alreadyRegistered.id}`));
      console.log(chalk.gray(`名前: ${alreadyRegistered.name}\n`));
    ...
    // Get global config for language default
    ...
    const projectConfig = {
      ...
      enabled: true,
      labels: {
        tasks: ['task:feature', 'task:bug', 'task:fix', 'task:docs', 'task:dogfooding'],
    ...
    await fs.writeFile(configPath, JSON.stringify({
      ...
      language: { primary: projectConfig.language }
    }, null, 2));
    ...
    console.log(chalk.gray(`  言語: ${projectConfig.language === 'ja' ? '日本語' : 'English'}`));
    console.log();
