...
class ListCommand {
  ...
  async execute(options) {
    try {
      ...
      console.log(colors.bold('📋 PoppoBuilder Projects'));
      console.log();
      ...
      if (projectIds.length === 0) {
        console.log(colors.yellow('No projects registered yet.'));
        console.log();
        console.log('To register a project:');
        console.log(colors.cyan('  poppobuilder init              # Initialize current directory'));
        console.log(colors.cyan('  poppobuilder project register  # Register existing project'));
        return;
      }
      ...
      if (filteredIds.length === 0) {
        console.log(colors.yellow('No projects match the specified filters.'));
        return;
      }
  ...
  async outputTable(projectIds, projects, options) {
    ...
    if (options.verbose) {
      headers.push('Issues', 'Errors', 'Last Activity');
    }

    // 列幅の計算
    const columnWidths = this.calculateColumnWidths(projectIds, projects, headers, options);
  ...
  showSummary(allProjects, filteredProjects, options) {
    ...
    if (filteredCount !== totalCount) {
      console.log(colors.gray(`Showing ${filteredCount} of ${totalCount} projects`));
    } else {
      console.log(colors.gray(`Total: ${totalCount} projects`));
    }
    
    console.log(colors.gray(
      `Enabled: ${colors.green(enabledCount)}  ` +
      `Disabled: ${colors.red(disabledCount)}`
    ));
    ...
    if (totalIssues > 0) {
      console.log(colors.gray(
        `Total processed: ${colors.cyan(totalIssues)} issues, ` +
        `${colors.red(totalErrors)} errors`
      ));
    }
  }
}
