  ...
  async runSetup(options = {}) {
    ...
    for (const step of this.steps) {
      ...
      if (step.autoFix && !options.skipAutoFix) {
        console.log(chalk.yellow(`Attempting automatic setup...`));
        const autoFixed = await step.autoFix();
        if (autoFixed) {
          const isValidAfterAutoFix = await step.validator();
          if (isValidAfterAutoFix) {
            console.log(chalk.green(`✓ ${step.title} configured automatically`));
            continue;
          }
        }
      }

      // Claude CLI guide is disabled due to compatibility issues with raw mode
      // Directly fall back to manual instructions or inquirer wizard
  ...
  async autoFixGitSetup() {
    try {
      ...
      try {
        ...
        if (!remotes) {
          // Can't auto-add remote without user input
          return false;
        }
      } catch {
        return false;
  ...
  async autoFixGhSetup() {
    try {
      ...
      try {
        ...
        try {
          execSync('claude --version', { stdio: 'ignore' });
          console.log(chalk.cyan('Claude CLIを使用して認証セットアップを開始します...'));
          
          const setupPrompt = `あなたはGitHub CLIのセットアップウィザードです。ユーザーがgh auth loginを実行して認証を完了できるようにガイドしてください。

以下のステップでガイドしてください：

1. まず「gh auth login」を実行してもらう
2. 認証方法の選択（通常はGitHub.com）
3. プロトコルの選択（HTTPSを推奨）
4. 認証方法（ブラウザがおすすめ）
5. 各ステップで何を選択すべきか、なぜその選択が良いのか説明
6. 認証が完了したら「gh auth status」で確認

日本語で丁寧に説明し、初心者でも分かりやすくガイドしてください。`;

          const success = await this.runClaudeGuide(setupPrompt);
          
          if (success) {
            // Verify authentication after Claude guide
            try {
              execSync('gh auth status', { stdio: 'ignore' });
          ...
          console.log(chalk.yellow('Claude CLIが利用できません。手動でセットアップしてください。'));
          return false;
        }
      }
    } catch (error) {
