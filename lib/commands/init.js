...
class InitCommand {
  ...
  async execute(options) {
    // Silence LogRotator during interactive init
  ...
  async registerProject(projectDir, config, options) {
    try {
      ...
      const projectConfig = buildProjectConfig({
        ...
        schedule: options.checkInterval ? {
          checkInterval: parseInt(options.checkInterval)
        } : undefined,
        resources: (options.maxConcurrent || options.cpuWeight || options.memoryLimit) ? {
          maxConcurrent: options.maxConcurrent ? parseInt(options.maxConcurrent) : undefined,
          cpuWeight: options.cpuWeight ? parseFloat(options.cpuWeight) : undefined,
          memoryLimit: options.memoryLimit
        } : undefined
      });

      const registrationOptions = buildRegistrationOptions(!options.disabled, projectConfig);
      const projectId = await projectRegistry.register(projectDir, registrationOptions);
