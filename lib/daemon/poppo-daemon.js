...
class PoppoDaemon extends EventEmitter {
  constructor(options = {}) {
    ...
    this.logger = new Logger('PoppoDaemon', { logDir: path.join(os.homedir(), '.poppobuilder', 'logs') });
  ...
  async startIPCServer() {
    ...
    this.ipcServer.on('command', async (message, connection) => {
      try {
        ...
        this.ipcServer.sendResponse(connection, message.id, result);
      } catch (error) {
        this.ipcServer.sendError(connection, message.id, error.message);
      }
    });
  ...
  setupLifecycleManager() {
    ...
    this.lifecycleManager.registerComponent('queue', {
      ...
      healthCheck: async () => this.queueManager?.isRunning || false
    ...
    this.lifecycleManager.registerComponent('api', {
      ...
      healthCheck: async () => this.apiServer?.isRunning || false
    ...
    this.lifecycleManager.registerComponent('ipc', {
      ...
      healthCheck: async () => this.ipcServer?.isRunning() || false
    }, { critical: false, startupOrder: 6 });
    
    // Task management components
    this.lifecycleManager.registerComponent('taskStatusTracker', {
      start: async () => {
      ...
      healthCheck: async () => this.taskStatusTracker !== null
    }, { critical: true, startupOrder: 7 });
    
    this.lifecycleManager.registerComponent('taskPriorityManager', {
      start: async () => {
        this.taskPriorityManager = new TaskPriorityManager();
      },
      healthCheck: async () => this.taskPriorityManager !== null
    }, { critical: true, startupOrder: 8 });
    
    this.lifecycleManager.registerComponent('taskRetryManager', {
      start: async () => {
      ...
      healthCheck: async () => this.taskRetryManager !== null
    }, { critical: true, startupOrder: 9 });
    
    this.lifecycleManager.registerComponent('taskResultHandler', {
      start: async () => {
      ...
      healthCheck: async () => this.taskResultHandler !== null
    }, { critical: true, startupOrder: 10 });
    
    this.lifecycleManager.registerComponent('taskScheduler', {
      start: async () => {
      ...
      healthCheck: async () => this.taskScheduler?.isRunning || false
    }, { critical: true, startupOrder: 11 });
    
    // Set up lifecycle events
    this.lifecycleManager.on('state-changed', (oldState, newState) => {
      this.logger.info(`Daemon state changed: ${oldState} -> ${newState}`);
      this.emit('state-changed', oldState, newState);
    });
    
    this.lifecycleManager.on('component-unhealthy', (name) => {
  ...
  setupWorkerPoolEvents() {
    ...
    this.workerPool.on('task-complete', async (result) => {
