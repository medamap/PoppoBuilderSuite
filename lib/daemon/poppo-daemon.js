...
class PoppoDaemon extends EventEmitter {
  ...
  async getStatus() {
    return {
      ...
      components: {
        ...
        api: this.apiServer?.isRunning || false,
        ipc: this.ipcServer?.isRunning() || false
      }
    };
  }
  ...
  setupLifecycleManager() {
    ...
    this.lifecycleManager.registerComponent('queue', {
      ...
      healthCheck: async () => ({ healthy: this.queueManager?.isRunning || false })
    ...
    this.lifecycleManager.registerComponent('workers', {
      ...
      healthCheck: async () => {
        ...
        return { healthy: stats?.totalWorkers > 0 };
      }
    ...
    this.lifecycleManager.registerComponent('api', {
      ...
      healthCheck: async () => ({ healthy: this.apiServer?.isRunning || false })
    ...
    this.lifecycleManager.registerComponent('ipc', {
      ...
      healthCheck: async () => ({ healthy: this.ipcServer?.isRunning() || false })
