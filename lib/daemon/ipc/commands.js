...
class Commands {
  ...
  async _overallStatus(args, context) {
    ...
    const projectList = await this._projectList(args, { registry, projectManager });
    const workers = await this._workerStatus(args, { workerPool });
    const queue = await this._queueStatus(args, { queueManager });
    
    // Convert project array back to object keyed by project ID
    const projectsObj = {};
    if (projectList.projects && Array.isArray(projectList.projects)) {
