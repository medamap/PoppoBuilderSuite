...
class IPCServer extends EventEmitter {
  constructor(options = {}) {
    ...
    this.authToken = options.authToken || uuidv4();
    this.server = null;
  ...
  _getDefaultSocketPath() {
    if (process.platform === 'win32') {
      return '\\\\.\\pipe\\poppo-daemon';
    }
    
    const tmpDir = process.env.TMPDIR || '/tmp';
    return path.join(tmpDir, 'poppo-daemon.sock');
  }
  ...
  async _processMessage(client, message) {
    try {
      ...
      if (message.type === 'command') {
        const result = await this.commands.execute(message.command, message.args, {
          daemon: this.daemon,
          clientId: client.id
        });
