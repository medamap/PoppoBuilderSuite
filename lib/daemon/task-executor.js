...
class TaskExecutor extends EventEmitter {
  ...
  registerDefaultHandlers() {
    ...
    this.registerTaskHandler('issue-processing', async (task, context) => {
      return await this.processIssue(task, context);
    });
    
    // Comment processing handler
    this.registerTaskHandler('comment-processing', async (task, context) => {
      return await this.processComment(task, context);
    });
    
    // Pull request processing handler
    this.registerTaskHandler('pr-processing', async (task, context) => {
  ...
  async createProjectComponents(projectId, projectPath, projectConfig) {
    ...
    // Initialize state manager to ensure lock directory exists
    ...
    // Create process manager with default config
  ...
  async processIssue(task, context) {
    ...
    // Create issue object from task data
    const issue = {
      number: issueNumber,
      title: task.title,
      body: task.body,
      labels: task.labels ? task.labels.map(label => ({ name: label })) : [],
    ...
    // Initialize StatusManager if needed
    if (!components.statusManager.isInitialized) {
      await components.statusManager.initialize();
    }
    ...
    try {
      ...
      // Prepare instruction for IndependentProcessManager
      const instruction = {
        type: 'issue',
        issue: {
          number: issueNumber,
      ...
      // Execute with Claude using IndependentProcessManager
      const result = await components.processManager.execute(
        `${task.type}-${issueNumber}-${Date.now()}`,
        instruction
      );
      ...
      if (result.success) {
        const commentText = result.response || result.output || 'タスクが完了しましたが、出力がありません。';
      ...
      console.error(`[TaskExecutor] Error processing task:`, error);
      
      // Reset issue status on error
      try {
        await components.statusManager.resetIssueStatus(issueNumber);
      } catch (resetError) {
        console.error(`[TaskExecutor] Failed to reset issue status:`, resetError);
      }
