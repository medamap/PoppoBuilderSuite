...
class InteractivePrompts {
  ...
  async confirm(questionKey, options = {}) {
    ...
    const yesNoDisplay = t(`prompts:yesNo.${yesNo}`);
    
    // Build the question with yes/no indicator
    const rl = this.createInterface();
    const questionText = t(questionKey, options.context || {});
    const prompt = `${questionText} ${chalk.gray(yesNoDisplay)}: `;
    ...
    if (!answerStr && defaultValue !== null) {
      return defaultValue;
    }
