...
class TimeFormatter {
  ...
  static formatRelativeTime(dateStr) {
    ...
    try {
      ...
      if (days > 0) {
        return `${days}${t(I18N_KEYS.TIME.days) || 'd'}${t(I18N_KEYS.TIME.ago) || ' ago'}`;
      } else if (hours > 0) {
        return `${hours}${t(I18N_KEYS.TIME.hours) || 'h'}${t(I18N_KEYS.TIME.ago) || ' ago'}`;
      } else if (minutes > 0) {
        return `${minutes}${t(I18N_KEYS.TIME.minutes) || 'm'}${t(I18N_KEYS.TIME.ago) || ' ago'}`;
      } else {
