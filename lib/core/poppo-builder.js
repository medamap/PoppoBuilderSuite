...
class PoppoBuilder extends EventEmitter {
  ...
  initializeManagers() {
    ...
    const LoggerAdapter = require('../utils/logger-adapter');
    ...
    this.logger = new LoggerAdapter('PoppoBuilder', { 
      logDir,
      projectId: this.config.projectId || path.basename(this.projectRoot),
      logLevel: this.config.logLevel || 'info'
    });
    
    // Register project with MultiLogger
    if (this.config.projectId && this.projectRoot) {
      this.logger.registerProject(this.config.projectId, this.projectRoot)
        .catch(err => console.error('Failed to register project with logger:', err));
    }
